...
def query_song_total(r_node):
	if globs.still_on_chord:
		if globs.first_on_query:
			globs.first_on_query = False
			return globs.my_ip + ":" + globs.my_port + "is responsible for the above songs" + json.dumps({'matrix_of_items':globs.songs})) + "\n"
		if config.NDEBUG:
			print("got a request for printing all my songs from")
			print(yellow(r_node))
		try:
			result = requests.post(config.ADDR + globs.nids[1]["ip"] + ":" + globs.nids[1]["port"] + ends.n_query_total, data = {"uid": globs.my_id, "ip": globs.my_ip, "port": globs.my_port})
			return globs.my_ip ":" + globs.my_port + "is responsible for the above songs" + json.dumps({'matrix_of_items':globs.songs})) + "\n" + result.text
		except:
			return "Node " + globs.nids[1]["uid"] + "  " + globs.nids[1]["ip"] + ":" + globs.nids[1]["port"] + " is not responding"
